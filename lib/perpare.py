...
from models.GAN import NetG, _NetG, NetD, NetC, _NetC

###########   preparation   ############
def prepare_models(args, new_NetG=False, new_NetC=False):
    device = args.device
    ...
    # GAN models
    if new_NetG == False:
        netG = _NetG(args.nf, args.z_dim, args.cond_dim, args.imsize, args.ch_size).to(device)
    else:
    ...
    if new_NetC == False:
        netC = _NetC(args.nf, cond_dim=args.cond_dim).to(device)
    else:
        netC = NetC(args.nf, cond_dim=args.cond_dim).to(device)
