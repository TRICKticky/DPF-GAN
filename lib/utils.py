...
#截断正态分布就是正态分布自变量x的取值范围从负无穷到正无穷值至左截断、右截断或左右截断（如这里的[-2,2]就是左右截断）
#使用截断正态分布主要是为了减少死亡神经元的产生，通常应使用少量噪声来初始化权重，以防止对称性破坏并防止0梯度。由于我们使用的是ReLU神经元，因此以略为正的初始偏差初始化它们也是一种好习惯，以避免“死神经元”。
#随机噪声的取值范围越大，越容易产生偏移（不对称，例如抛硬币10次，可能有8次都是正面，就是抛的次数太少了，犹如随机噪声z的维度大小一样，维度(取样的数量)太小，也容易一边倒），假如向负值偏移，那么对于relu层，很容易产生死亡细胞（bp学不到东西）
#同理，在训练时使用正常的非截断的分布，测试时使用截断的分布，就是一个数据量大小的问题
